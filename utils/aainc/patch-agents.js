 ...
 * だけ末尾マーカーを追加する。Anthropic の上限 4 箇所（system 1 + 安定 prefix 2 + 末尾 1）に収まる。
 *
...
for (const { rel, fn } of TARGETS) {
  ...
  fs.writeFileSync(file, src.replace(before(), after(fn)));
