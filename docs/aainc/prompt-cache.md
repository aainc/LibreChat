...
| agents の動的 tail 以降にも末尾マーカーを打つパッチ | `utils/aainc/patch-agents.js`（postinstall で適用、`patch-agents.check.js` で確認） | 本家は動的 instruction（メモリ等）がある run で安定 prefix にしかマーカーを置かず、ツール往復ごとに過去のツール出力が全量キャッシュなしで再送される。安定 prefix のマーカーは残したまま、動的 tail 以降の区間に末尾マーカーを足す |
