...
const { generateAgentToken } = require('~/server/services/Agents/auth');
...
const createFileSearchTool = async ({ req, files, entity_id }) => {
  return tool(
    async ({ query }) => {
      ...
      // If entity_id (agent_id) is provided, generate an agent token
      if (entity_id) {
        try {
          // Generate a token that represents the agent as the owner
          jwtToken = generateAgentToken(entity_id);
          logger.debug(`[${Tools.file_search}] Using agent token for entity_id: ${entity_id}`);
        } catch (error) {
          logger.error(`[${Tools.file_search}] Error generating agent token: ${error.message}`);
          // Fall back to user token if agent token generation fails
